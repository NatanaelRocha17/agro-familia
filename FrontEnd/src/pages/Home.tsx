...
export function Home() {
  ...
  const [selectedRadius, setSelectedRadius] = useState<number | null>(
    initialFilters?.selectedRadius ?? DEFAULT_RADIUS,
  );
  ...
  const saveFiltersToStorage = useCallback(
    (overrides: Partial<PersistedFilters> = {}) => {
      ...
      try {
        ...
        const finalSearch =
          overrides?.searchTerm !== undefined
            ...
            : appliedSearchTerm;
        const finalPriceMin =
          overrides?.priceMin !== undefined ? overrides.priceMin : priceMin;
        const finalPriceMax =
          overrides?.priceMax !== undefined ? overrides.priceMax : priceMax;
        const finalCategoryId =
          overrides?.categoryId !== undefined
            ? overrides.categoryId
            : categoryId;
        const finalRadius =
          overrides?.radius !== undefined ? overrides.radius : selectedRadius;
  ...
  const handleApplyFilters = useCallback(async () => {
    if (!userLocation)
      return toast.error("Precisamos da sua localização primeiro");
  ...
  const removeFilter = useCallback(
    async (filterType: "category" | "radius" | "price" | "search") => {
      if (!userLocation) return;
  ...
  return (
    <div id="inicio" className="min-h-screen bg-stone-50 pb-20">
      ...
      >
        ...
        {/* Mantive flex-row para que fiquem lado a lado desde o mobile */}
        <div className="bg-white p-4 rounded-xl shadow-lg mb-8 flex flex-row gap-3 items-center">
          <button
            onClick={() => setShowFilters(!showFilters)}
            className={`flex items-center gap-2 px-4 py-3 rounded-xl border transition-all shadow-sm whitespace-nowrap
      ${showFilters ? "bg-green-700 text-white border-green-700 shadow-md" : "bg-white text-stone-600 border-stone-200 hover:bg-stone-50"}`}
          ...
          <div className="relative flex-1">
            <input
              className="w-full pl-2  py-3 border border-stone-200 rounded-xl outline-none focus:border-green-500 focus:ring-1 focus:ring-green-500 transition-all"
            ...
            <div className="flex items-center gap-3">
              {!userLocationOrCep ? (
                <>
                  ...
                  <div>
                    <p className="font-bold text-blue-900 text-sm">
                      Produtos baseados no CEP
                    </p>
                    <p className="text-blue-700 text-sm">
                      CEP {userCep.replace(/(\d{5})(\d{3})/, "$1-$2")} •
                      Mostrando produtos da sua região
                    </p>
                  </div>
                </>
              ) : (
                <>
                  ...
                  <div>
                    <p className="font-bold text-blue-900 text-sm">
                      Produtos baseados na localização atual
                    </p>
                    <p className="text-blue-700 text-sm">
        ...
        {isGettingLocation || isLoadingProducts ? (
          <div className="flex flex-col items-center justify-center py-24">
            <Loading />
            <p className="text-stone-700 text-lg font-medium text-center">
              Estamos localizando produtos perto de você
            </p>
          </div>
        ) : products.length === 0 ? (
